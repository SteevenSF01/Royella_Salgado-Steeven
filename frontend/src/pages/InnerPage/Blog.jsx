...
const Blog = () => {
    ...
    useEffect(() => {
        const fetchData = async (page = 1, title = "", category = null) => {
            const categoryQueryParam = category ? `&categorie=${category}` : "";
            const response = await axios.get(
                `/api/backoffice/blog?page=${page}&title=${title}${categoryQueryParam}`
            );
            setBlogData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 6));
        };
        fetchData(currentPage, filterTitle, filterCategory);
    }, [currentPage, filterTitle, filterCategory]);
            ...
            <div className="dark:bg-lightBlack py-20 2xl:py-[120px]">
                <div className="Container grid grid-cols-6 md:grid-cols-7 lg:grid-cols-6 gap-5 ">
                    <div className="col-span-6 md:col-span-4">
                        ...
                        //{" "}
                        <Link to="/create_blog">
                            <button className="bg-khaki hover:bg-[rgba(141,115,77,0.9)] text-white font-bold py-2 px-4 rounded mt-5">
                                Create a new blog
                            </button>
                        </Link>
                        {/* Pagination */}
                    ...
                    {/* Blog Sidebar */}
                    <div className="col-span-6 md:col-span-3 lg:col-span-2 order-first md:order-last">
                        <BlogSideBar
